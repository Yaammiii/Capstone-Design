...
def send_ctrl_info(
    ...
    rclpy.init()
    node = rclpy.create_node('PC_node1')
    publisher = node.create_publisher(Float32MultiArray, 'steer_info1', 5)
...
def lane(region, buffer_Handle, buffer_Front_Img):
    ...
    sub = RSPi_Subsriber(node_name = 'RSPiSub_Node1',
                        topic_name = 'Lane_Img1'
                        )
    ...
    while True:
        ...
        frame_R = merged_img[:, 0:320, :]
        frame_Front = merged_img[:, 320:640, :]
        frame_L = merged_img[:, 640:960, :]
