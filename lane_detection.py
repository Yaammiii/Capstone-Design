...
def color_and_gradient_threshold(img, side):
    ...
    # th, img = cv2.threshold(blurred, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    # gaussian = img
...
def curvatures(y_pix_pos, x_pix_pos, ploty):
    ...
    if len(x_pix_pos) <= 1:
        ...
        curverad = np.arctan(fit_cr[0]) * 180 / np.pi + 90
        ...
        return curverad, fit_cr[1]

def back_warp(img_shape, warped, left_fit, ploty, curverad, Minv, fitcr):
    ...
    TextL = f"Curvature: {int(curverad)} / {fitcr}"
    fontScale=0.8
    thickness=1
    fontFace = cv2.FONT_HERSHEY_COMPLEX

    # Using CV2 putText to write text into images
    # cv2.putText(result, TextL, (5,20), fontFace, fontScale,(0,255,255), thickness,  lineType = cv2.LINE_AA)
...
def pipeline(img, region, prev_curvature, prev_fit, side):
    ...
    try:
        ...
        # print('1111111111')
        # print('draw tangential line error', 1)
        angle=1

    # Calulating the left and right lines curvatures
    try:
        curverad, fitcr = curvatures(y_pix_pos, x_pix_pos, ploty)
    ...
    result_lane = back_warp((img.shape[0], img.shape[1]), out_img, left_fit, ploty, curverad, Minv, fitcr)
    return result_lane, curverad, left_fit, angle, fitcr
